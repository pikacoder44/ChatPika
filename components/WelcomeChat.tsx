...
const WelcomeChat = ({ onPick }: WelcomeChatProps) => {
  const messages = [
    "Write a creative and engaging invitation message for a birthday party, including date, time, location, and RSVP details.",
    "Suggest a detailed study plan for exam preparation, including daily schedules, revision techniques, and effective time management tips.",
    "Tell me the best time of year to travel to Japan, considering weather, festivals, tourist crowds, and budget-friendly options.",
  ];

  const handlePick = (message: string) => {
    if (onPick) {
      onPick(message);
    } else {
    ...
    <div className="mx-auto flex w-full flex-col justify-center px-2 pb-2 transition-all duration-700 max-w-3xl flex-1">
      <div className="flex flex-col items-center justify-center pb-4 text-center">
        ...
        <div className="grid w-full auto-rows-min gap-4 md:grid-cols-3">
          <button onClick={() => handlePick(messages[0])}>
            <div className="card bg-indigo-500/10 hover:bg-indigo-500/15 border-indigo-500/10 text-indigo-800 dark:text-indigo-200  cursor-pointer ">
              <div className="flex flex-row items-center gap-1">
                ...
                <span>Write an invitation</span>
              </div>
              <p>Prepare an invitation for an event or a party</p>
            </div>
          </button>
          <button onClick={() => handlePick(messages[1])}>
          ...
          <button onClick={() => handlePick(messages[2])}>
